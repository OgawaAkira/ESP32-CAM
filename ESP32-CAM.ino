...
void setup() {
  ...
  server.on("/", HTTP_GET,[](AsyncWebServerRequest *request){
    ...
    html += "<button onclick=\"fetch('/capture').then(() => alert('Foto tirada com sucesso!'))\" style='padding:15px; font-size:18px; margin:10px; cursor:pointer;'>TIRAR FOTO</button><br>";
  ...
  server.on("/galeria", HTTP_GET,[](AsyncWebServerRequest *request){
    ...
    // Botão e Script para baixar todas as fotos de uma vez
    ...
    html += "  alert('Iniciando download de ' + links.length + ' fotos. Aguarde...');";
    html += "  links.forEach(link => {";
    html += "    setTimeout(() => {";
    html += "      let a = document.createElement('a');";
    html += "      a.href = link.href;";
    html += "      a.download = link.innerText;"; // Força o download com o nome do arquivo
    html += "      document.body.appendChild(a);";
    html += "      a.click();";
    html += "      document.body.removeChild(a);";
    html += "    }, delay);";
    html += "    delay += 800;"; // Dá um intervalo de 800ms entre cada download para não travar o ESP32
    html += "  });";
    html += "}";
    html += "</script>";
    ...
    while(file){
      ...
      if(fileName.indexOf(".jpg") > 0){
        ...
        html += "<img src='/verfoto?nome=" + fileName + "' width='300'>";
        html += "</div><br>";
      }
      file = root.openNextFile();
    }
